  ...
  render() {
    ...
    return (
      <div className='sm:w-[92%] p-2 mt-3 mx-auto flex flex-col border-2 border-gray-200 bg-white'>
        <h1 className='text-left my-3 text-md font-bold'>Groceries Products</h1>
        <Carousel responsive={responsive}>
          {products && products.filter(item => item.category.toLowerCase().includes('groceries')).map((item) => (
            <div
              key={item.id}
